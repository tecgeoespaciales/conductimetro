...
class LaboratoryCalibrationDialog(QDialog):
    ...
    def _finish_measurement(self):
        ...
        Flujo:
        1) Obtiene voltaje representativo (moda) y temperatura promedio.
    ...
    def save_laboratory_calibration(self, archivo_cfg="calibration_ranges.cfg"):
        ...
        try:
            ...
            if self.replace_existing:
                ...
                if cfg.has_section('CALIBRATION_K_TABLE'):
...
class UnknownCalibrationTableDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.table.setColumnCount(6)
        expected_headers = ["Conductividad Conocida (µS)", "Temperatura (°C)", "Botón Medir", "Valor Medido (V)", "", "Estimado (µS)"]
    ...
    def _finish_measurement(self):
        """Finaliza la captura de una fila y consolida su punto de calibración.

        Flujo técnico:
        1) Valida muestras capturadas y obtiene voltaje representativo (moda tolerante).
        2) Estima temperatura de la muestra con fallback seguro.
        3) Calcula EC base por polinomio y obtiene K directo: K = EC_conocida / EC_polinomio.
        4) Guarda el punto en memoria/configuración y actualiza estado visual.

        Side effects:
            - Actualiza `self.calibration_data` y tabla KNOWN_K_TABLE.
            - Opcionalmente sincroniza comandos seriales con el ESP32.
        """
        row = self._sampling_row
        known_cond = self._sampling_known_cond
        samples = [s for s in self._sampling_samples if s is not None]
        ...
        # Usa promedio de temperaturas capturadas; si no existe, usa última lectura global.
        ...
        # Acota temperatura para evitar propagar valores corruptos del sensor.
        if temp_read < -40 or temp_read > 125:
            temp_read = temp_ref

        # En valores conocidos se usa voltaje crudo; la normalización a 25°C
        # aplica únicamente al flujo de calibración de laboratorio.
        ec_measured = POLY_A * measured_value**3 - POLY_B * measured_value**2 + POLY_C * measured_value

        # K se calcula directo para no contaminar con tablas históricas/interpoladas.
        k_direct = known_cond / ec_measured if ec_measured > 0 else 1.0

        k_value = k_direct

        # Estimado final usado para validar coherencia de la captura.
        estimated = k_value * ec_measured
        ...
        # Recalcula rangos completos para mantener consistencia global de calibración.
        try:
        ...
        try:
            ...
            section_name = f"KNOWN_K_TABLE"

            # Se persiste como Voltaje->K para que la interpolación opere en dominio de señal.
        ...
        # Fuerza recomputo local para alinear UI con la calibración recién guardada.
        try:
...
class ESP32App(QWidget):
    ...
    def open_calibration_update(self):
        ...
        reply = QMessageBox.warning(
            ...
            "Solo proceda si está autorizado para calibrar el equipo.",
        ...
        if password_dialog.exec_() == QDialog.Accepted:
            ...
            decision_dialog.setIcon(QMessageBox.Question)
            decision_dialog.setWindowTitle("Gestión de Datos de Laboratorio")
            decision_dialog.setText(
                "¿Desea borrar los datos de laboratorio existentes y generar una nueva curva, "
                "o desea agregar nuevos datos a los ya existentes?"
            )

            replace_button = decision_dialog.addButton(
                "Borrar y generar nueva curva", QMessageBox.AcceptRole
            )
            append_button = decision_dialog.addButton(
                "Agregar datos", QMessageBox.ActionRole
            )
            cancel_button = decision_dialog.addButton(
                QMessageBox.Cancel
            )
            decision_dialog.setDefaultButton(append_button)
            ...
            replace_existing = clicked == replace_button

            # Contraseña correcta, abrir calibración de laboratorio
            lab_dialog = LaboratoryCalibrationDialog(
                self,
                num_points=LAB_CALIBRATION_CONFIG['num_points'],
    ...
    def _transition_calibration_mode(self, target_mode: str):
        ...
        previous_mode = self.calibration_mode
        previous_use_user = self.use_user_calibration
        previous_ranges = dict(getattr(self, 'known_k_ranges', {}) or {})
        previous_table = dict(getattr(self, 'known_k_table', {}) or {})
        try:
            ...
            if target_mode == "known":
                ...
                try:
                    loaded = self.load_known_k_table(force_load=True)
                except Exception:
                    loaded = {}

                if not loaded:
                    raise RuntimeError("No hay tabla de 'Valores conocidos' disponible.")
            else:
                self.use_user_calibration = False
                try:
                    loaded = self.load_known_k_table(force_load=True)
                ...
                if not loaded:
                    raise RuntimeError("No hay tabla de 'Valores de laboratorio' disponible.")
            ...
            # Rollback completo para evitar mezclar fuentes entre modos.
            self.calibration_mode = previous_mode
            self.use_user_calibration = previous_use_user
            self.known_k_ranges = previous_ranges
            self.known_k_table = previous_table
            try:
                self.set_calibration_mode_ui(previous_mode, send_to_device=False)
            except Exception:
                pass
            try:
    ...
    def download_csv(self):
        """Exporta una copia del CSV local a una carpeta elegida por el usuario.

        Si no existe archivo de origen, notifica error y aborta la operación.
        """
        try:
    ...
    def plot_csv_data(self):
        """Carga un CSV y abre la ventana de visualización histórica.

        El parser usa columnas por índice (sensor=2, temperatura=3), compatible
        con el formato actual sin K y con versiones previas que tenían columnas extra.
        """
        try:
